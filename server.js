...
client.on("message", async message => {
  ...
  if (message.content.match(prefixMention)) {
    return message.reply(`Mój prefix to: \`${default_prefix}\``);
  }
...
client.on("guildMemberAdd", async member => {
  ...
  client.channels.cache.get(chx).send("Witaj na serwerze! " + member.user.username, attachment);
...
client.on("guildCreate", guild => {
  ...
  const embed = new MessageEmbed()
    ...
    .addField(`**Właściciel**`, `\`\`\`${sowner.tag}\`\`\``)
...
client.on("ready", () => {
    ...
    client.user.setActivity(`${client.users.cache.size} użytkowników!`, { type: "WATCHING"})
});
client.login("ODA1MTQ2NTk4MzA0NjQ1MTcx.YBWpMw.9FhelPXx0DRqwHBJxwG-4Z7Crvc" );
